...
pub fn set_vrm_scene(
    ...
    scenes: Query<(Entity, &mut SceneRoot, &HVrm), With<AutoScene>>,
    vrm: Res<Assets<Vrm>>,
) {
    for (entity, scene_handle, vrm_handle) in scenes.iter() {
        ...
        let vrm_scene = match gltf_scenes.get(vrm_scene) {
            Some(scene) => SceneRoot(scene.scene.clone()),
            None => continue,
        };
        ...
        commands.entity(entity).insert(vrm_scene);
    }
}
